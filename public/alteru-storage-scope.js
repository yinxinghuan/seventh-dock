...
(function installAlteruStorageScope(global) {
  ...
  function prefix() { return 'alteru:' + currentGameId() + ':' }
  ...
  function adapter(storage) {
    return {
      get length() { return scopedKeys(storage).length },
      clear: function clear() { scopedKeys(storage).forEach(function remove(key) { storage.removeItem(key) }) },
      getItem: function getItem(key) { return storage.getItem(prefix() + String(key)) },
      key: function key(index) {
        var stored = scopedKeys(storage)[index]
        return stored ? stored.slice(prefix().length) : null
      },
      removeItem: function removeItem(key) { storage.removeItem(prefix() + String(key)) },
      setItem: function setItem(key, value) { storage.setItem(prefix() + String(key), String(value)) },
    }
  }
