...
`usePlayerProfile()` 通过 `/note/telegram/user/get/info/by/telegram_id` 读取 `data.name` 与 `head_url`。每张场景图保存 `player / environment / others` 主体分类；只要玩家主角在画面任何位置出现，包括远景中执行动作，就把 HTTPS 头像作为 `ref_url`。只有无人空镜或只出现其他角色时不传。头像与用户名不进入 StorySave。
...
声音在首次用户手势后启动。普通选择只播放一次轻确认；普通正文、常规数值和图片完成静音，检定、稀有物、关系、危险、抵达与章节节点才追加结果 cue。`music / ambient / sfx` 三路总线中 SFX 额外乘 `0.52`，`180 ms` 内合并突发触发，合成/录制短音效并发上限为 6/2。顶部 44 px 扬声器按钮按实际 `ready` 状态显示开关；页面后台暂停，设备不支持时静默降级，音频偏好不进入 StorySave。
...
港区常规阅读使用稀疏区域底乐 A；关键发现、关系转折和阶段小结才调用更完整的 B。B 播放时暂停 A、结束后恢复，同源 B 冷却至少 180 秒；未知地点继续安全回落默认环境声。
