...
- 正式入口默认由 Aigram `game-chat` 根据当前存档持续生成；`?story_mode=demo` 仅用于有限 QA，带有效 `chat_id` 时使用实验连续世界。
...
调试头像：`?avatar_url=<public-https-url>&user_name=Alex`。调试语言：`?lang=en` 或 `?lang=zh`。确定性演示：`?story_mode=demo`。
