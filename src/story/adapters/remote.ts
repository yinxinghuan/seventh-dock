...
export const remoteAdapter: StoryAdapter = {
  ...
  async send(action, context, onProgress): Promise<AdapterResult> {
    ...
    const languageInstruction = context.locale === 'en'
      ...
      : `\n\n[语言与格式要求：请用简体中文回复，并保持所有协议命令标签及语法不变。每次回复（包括真正的章节节点）都必须在结尾用这一机器可读格式给出恰好三个行动：[choices: "行动一"|"行动二"|"行动三"]。按钮行动必须与正文描述的决定一致。遇到具有明显视觉价值的新地点、发现、关系转折、重大结果或阶段节点时，用 [image_prompt: "English cinematic visible scene, no text, no UI, 4:3"] 提议一张场景图，并紧接着输出 [image_subject: "player|environment|others"]。画面任何位置出现玩家主角（包括远景）都必须用 player；玩家执行画面主动作时也必须用 player；environment 表示无人空镜，others 表示有人但玩家不在。只画正文已经公开的事实，并遵循这一画风：${sceneImageDirection}。${imageFreshness} 普通对话不要提议。]`
