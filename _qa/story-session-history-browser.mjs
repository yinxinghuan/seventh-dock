...
const root=resolve(dirname(fileURLToPath(import.meta.url)),'..'); const databaseDirectory=await mkdtemp(join(tmpdir(),'seventh-dock-history-ui-')); const evidenceDirectory=resolve(root,'_qa/ui'); await mkdir(evidenceDirectory,{recursive:true})
const child=spawn(process.execPath,['--import','tsx','_qa/run-story-session-ui.ts'],{cwd:root,env:{...process.env,STORY_LAB_UI_DATABASE_DIR:databaseDirectory,STORY_LAB_UI_PORT:'5189'},stdio:['ignore','pipe','pipe']})
...
try{
  ...
  await page.locator('.st-session-history').evaluate(node=>node.scrollIntoView({block:'start'}));await page.screenshot({path:join(evidenceDirectory,'platform-layout-seventh-dock-session-history-zh-390x844.png'),fullPage:true});await page.locator('.st-session-history__list button:not([aria-current="true"])').click();await page.waitForFunction(expected=>document.querySelector('output[data-story-session-test]')?.getAttribute('data-session-id')===expected,original);await waitIdle(page)
