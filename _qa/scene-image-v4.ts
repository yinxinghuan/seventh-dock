...
const save = applyParsedScene(createInitialSave(cartridge), scene, cartridge, '打开箱子', cartridge.opening.imagePrompt, 'player')
...
if (image?.data?.playerVisible !== 'true') throw new Error('acting player was not marked visible')
...
if (!shouldUsePlayerImageReference('wide shot with the player protagonist crossing a bridge')) throw new Error('wide player shot should keep the player reference')
