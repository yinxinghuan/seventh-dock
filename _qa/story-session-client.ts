...
const directory = await mkdtemp(join(tmpdir(), 'seventh-dock-story-session-'))
...
try {
  ...
  const store = new Store(); const scope = 'seventh-dock:deployment:owner-a:zh'
  ...
  const foreign = await client('token-b').enroll(createInitialSave(seventhDock), 'owner-b-enrollment')
  ...
  const rollbackSession = await client().enroll(createInitialSave(seventhDock), 'rollback-enrollment')
